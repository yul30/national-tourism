...
// Определяем интерфейс для локации прямо в файле
interface Location {
  id: string;
...
function SortableItem({ item }: { item: Location }) {
  ...
  return (
    <div
      ...
      className="p-4 border rounded-md flex justify-between items-center hover:shadow transition-shadow cursor-move"
    >
      <div>
      ...
      <div className="text-sm text-gray-600">День {item.order + 1}</div>
    </div>
  );
}
